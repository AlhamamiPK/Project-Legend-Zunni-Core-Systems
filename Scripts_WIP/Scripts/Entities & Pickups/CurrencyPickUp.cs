...
{
    [Header("Assign your ScriptableObject here!")]
    ...
    [Header("Physical Parts Of Currency")]
    ...
    public float minScale = 1.0f; // Adjusted for coins, tweak as needed!
    public float maxScale = 1.5f;
    public float flightAcceleration = 35f;
    public bool addSparkleParticles = true;
    public Color sparkleColor = Color.yellow; // Choose a nice shiny color in the inspector

    public SpriteRenderer spriteRenderer;
    public ParticleSystem sparkleParticles;
    public bool isCollected = false;

    // Flight and Homing Variables
    public Transform playerTransform;
    public float baseFlySpeed;
    public float currentFlySpeed;
    public bool isFlyingToPlayer = false;
    public bool canBeMagnetized = false;
    ...
    void Start()
    {
        ...
        float randomWaitTime = Random.Range(0.6f, 1.5f);
        ...
        StopAllCoroutines();
        StartCoroutine(DelyToTurnOnMagnet());
    }
    void Update()
    {
        if (canBeMagnetized && !isFlyingToPlayer && playerTransform != null && !isCollected)
        {
            ...
            // If the player steps inside the radius, trigger the homing missile!
        ...
        // 2. THE HOMING MISSILE LOGIC (Runs after StartFlying is triggered)
    ...
    IEnumerator DelyToTurnOnMagnet()
    {
    ...
    void StartFlying()
    {
        ...
        if (RB != null)
        {
            ...
            //Debug.Log("Gravity disabled successfully!");
        }
        else
        {
            // This will flash a red error in your Unity Console if the slot is empty!
           // Debug.LogError("🚨 BUG: The RB slot is empty in the Inspector on " + gameObject.name + "! I cannot turn off gravity.");
        }
    }
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player") && !isCollected)
        {
            ...
            CurrencyManager.Instance.AddCurrencey((int)currencyData.currencyValue);

            if (currencyData.pickUpSoundForTheCurrency != null)
            {
                AudioSource.PlayClipAtPoint(currencyData.pickUpSoundForTheCurrency, transform.position);
            }
            collectFeedBack.PlayFeedbacks();
            spriteRenderer.enabled = false;
            // Optional: If you want the particle trail to finish its animation before vanishing
    ...
    private void SetupParticleSystem()
    {
        ...
        main.startSpeed = 0f;
        main.startSize = 0.5f;
        main.startColor = sparkleColor; // Set to the color chosen in the inspector
    ...
    {
        Gizmos.color = Color.yellow; // Draws a yellow circle around the coin
